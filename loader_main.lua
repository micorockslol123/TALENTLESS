...
bpmtext["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
...
upbpm["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
...
downbpm["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
...
errorbox["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
...
less["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
...
more["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
...
if not errormargin then errormargin = 0 else end
...
less.MouseButton1Click:Connect(function()
    if errormargin <= 0 then return end
    errormargin = round(errormargin - 0.005, 3)
    updateErrorMargin()
end)
...
local shiftKeys = {"!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "A", "S", "D", "F", "G", "H", "J", "K", "L", "Z", "X", "C", "V", "B", "N", "M"}
...
function pressKey(keys, beats, bpm)
    ...
    VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.LeftShift, false, game) -- make sure shift is off

    -- Press non-shift keys first
    for _, key in ipairs(nonShift) do

        local agf = errormargin * 100 -- so 0.01 is 1
        if math.random(1, 175) <= agf then -- 0.01 is 1/175 chance, 0.04 is 4/175 which is a 2.3%
        ...
        coroutine.wrap(function()
            VirtualInputManager:SendKeyEvent(true, keyMappings[key], false, game)
                local waittime = math.random(4, 12) / 100 -- random number from 0.044 to 0.15
                task.wait(waittime)
        ...
        VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.LeftShift, false, game) -- turn shift off just in case
    ...
    if #shiftRequired > 0 then
        ...
        for _, key in ipairs(shiftRequired) do
            ...
            local agf = errormargin * 100 -- so 0.01 is 1
            if math.random(1, 175) <= agf then -- 0.01 is 1/200 chance, 0.04 is 4/200
            ...
            coroutine.wrap(function()
            VirtualInputManager:SendKeyEvent(true, keyMappings[key], false, game)
            local waittime = math.random(4, 13) / 100
            task.wait(waittime)
            VirtualInputManager:SendKeyEvent(false, keyMappings[key], false, game)
            end)()
            ...
            VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.LeftShift, false, game) -- reshift in case
        end
